...
Theorem leq_n_pred n m : leq n m -> leq (pred n) (pred m).
Proof.
  induction 1; auto.
  destruct m; simpl; auto.
Defined.

Theorem leq_S_n : forall n m, n.+1 <= m.+1 -> n <= m.
Proof.
  intros n m.
  apply leq_n_pred.
Defined.
...
Existing Class lt.
(** Importantly we add hints to [core] and [typeclass_instances]. The first will ensure that tactics like [auto] will unfold [lt] and use what they know about [leq]. The second says that in order to do typeclass search on [lt] its a good idea to unfold it to [leq] and do a search there. *)
...
Global Instance transitive_gt : Transitive gt := fun x y z p q => transitive_lt _ _ _ q p.

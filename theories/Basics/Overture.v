...
(** We register wrappers for [transport] so that [rewrite] produces terms that unfold to [transport].  Their argument order preserves the standard ordering of goals generated by [rewrite]. *)
Definition paths_rew {A : Type} {x y : A} (P : A -> Type)
  (u : P x) (p : x = y)
  : P y
  := transport P p u.

Register Scheme paths_rew as rew for paths.

Definition paths_rew_r {A : Type} {x y : A} (P : A -> Type)
  (u : P y) (p : x = y)
  : P x
  := transport P p^ u.

Register Scheme paths_rew_r as rew_r for paths.

...
Section Localization.
  ...
  Lemma eq_fraction_plus (a c e : R) (b d f s : S)
  ...
  Proof.
    ...
    set (b' := mss_incl b) in *; set (d' := mss_incl d);
    set (f' := mss_incl f) in *; set (s' := mss_incl s).
    rewrite (rng_dist_l (f' * d')).
    rewrite (rng_dist_l (b' * d')).
    rewrite rng_negate_plus.
    rewrite rng_plus_assoc.
    transitivity (s' * (f' * a * (d' * d') - b' * e * (d' * d'))).
    { apply ap.
      rewrite <- (rng_mult_assoc f' d').
      rewrite (rng_mult_comm d').
      ...
      rewrite rng_plus_assoc.
      rewrite <- (rng_mult_assoc b' d' (e * _)).
      rewrite (rng_mult_comm d').
      rewrite <- (rng_mult_assoc e).
      rewrite (rng_mult_assoc b').
      refine (_ @ rng_plus_zero_l _).
      f_ap.
      rewrite <- rng_mult_assoc.
      rewrite (rng_mult_assoc d' b').
      rewrite (rng_mult_comm d' b').
      rewrite (rng_mult_comm _ c).
      rewrite rng_mult_assoc.
  ...
  Lemma eq_fraction_mult (a c e : R) (b d f s : S)
    (p : mss_incl s * (mss_incl f * a - mss_incl b * e) = 0)
    : mss_incl s * (mss_incl (f * d) * (a * c) - mss_incl (b * d) * (e * c)) = 0.
  Proof.
    rewrite ? mss_incl_mult, ? (rng_mult_comm _ c).
    rewrite ? (rng_mult_assoc _ c), <- ? (rng_mult_assoc _ _ c).
    rewrite ? (rng_mult_comm _ (_ * c)), <- ? (rng_mult_assoc (_ * c)).
    rewrite ? (rng_mult_comm (_ * c)), <- rng_mult_negate_l.
    rewrite <- (rng_dist_r _ _ (_ * c)), rng_mult_assoc.
    rewrite p.
    apply rng_mult_zero_l.
  Qed.
  ...
  Proof.
    ...
    rewrite ?mss_incl_mult.
    rewrite ?mss_incl_one.
    rewrite (rng_mult_comm (mss_incl _)).
    rewrite (rng_plus_comm (a * _)).
    rewrite (rng_mult_comm _ c).
    rewrite (rng_mult_comm a _).

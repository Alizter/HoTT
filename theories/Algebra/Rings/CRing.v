...
Section RingLaws.
  ...
  (** Negation of zero *)
  Definition rng_negate_zero : - 0 = 0 := negate_0 (R:=A).
...
Lemma rng_pow_mult {R : CRing} (n m : nat) (a : R)
  : rng_pow (n + m)%nat a = rng_pow n a * rng_pow m a.
Proof.
  induction n.
  { symmetry.
    apply left_identity. }
...
Section Indexed.
  ...
  Lemma rng_indexed_sum_invariant (n : nat) (a : Fin n -> R)
    (e : Fin n <~> Fin n)
    : rng_indexed_sum_fin n a = rng_indexed_sum_fin n (a o e).
  Proof.
    destruct n.
    1: reflexivity.
    revert e.
  ...
  Proof.
    ...
    { intro e.
      exact (rng_indexed_sum_fin _ (a o e^-1)). }
    ...
    set (a' := a o e1^-1).
    set (e' := e1 oE e2^-1).
    change (rng_indexed_sum_fin _ a' = rng_indexed_sum_fin _ (a' o e')).
    clearbody a' e'; clear e1 e2 a X.
    apply rng_indexed_sum_invariant.
  Defined.
